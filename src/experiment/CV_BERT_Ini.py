...
# Image Processor which is like Tokenizer in BERT
...
class CVBERT(nn.Module):
    ...
    def forward(self, image, attention_mask=None):
        ...
        # 2. adjust size of attention mask
...
class CustomEmbeddingLayer(nn.Module):
    ...
    def forward(
        ...
        embeddings = inputs_embeds  # [batch_size, num_patches, embed_dim]

        if position_ids is None:
            # Calculate the position embedding based on the patch
        ...
        position_embeddings = self.position_embeddings(position_ids)  # [batch_size, num_patches, embed_dim]
        if token_type_ids is None:
            token_type_ids = torch.zeros_like(position_ids, dtype=torch.long)

        token_type_embeddings = self.token_type_embeddings(token_type_ids)  # [batch_size, num_patches, embed_dim]
...
for fold, (train_idx, val_idx) in enumerate(kf.split(image_paths)):
    ...
    model = CVBERT(bert_model_name, num_classes=num_classes).to(device)
...
filename = "Result_CV_BERT_Ini.txt"
