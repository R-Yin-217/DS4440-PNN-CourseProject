...
image_features = torch.tensor(df.drop(columns=['file_path', 'label'], axis=1).values, dtype=torch.float)
...
class CVBERT(nn.Module):
    ...
    def forward(self, input_tensor, attention_mask=None):
        ...
        # BERTは [batch_size, sequence_length, hidden_size], sequence_length=1 
...
for fold, (train_idx, val_idx) in enumerate(kf.split(image_features)):
    ...
    # split data
    train_image_paths = image_features[train_idx]
    val_image_paths = image_features[val_idx]
    train_labels = image_labels[train_idx]
    val_labels = image_labels[val_idx]
